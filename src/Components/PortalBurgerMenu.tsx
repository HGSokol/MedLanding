...
export default function PortalBurgerMenu({ isOpen, setIsOpen }: PortalMenu) {
	...
	return createPortal(
		<div
			id="portalMenu"
			className={`${
				isOpen ? 'z-[100] translate-x-0' : 'z-[-1] -translate-x-[1000rem]'
			} overflow-y-scroll md:hidden fixed top-0 w-full max-h-[100vh] duration-500 text-[#242E49]`}>
			<div className="p-[16rem] flex flex-col w-full h-[100dvh] bg-[#EDF0F4] ">
				<div className="flex h-[46rem] mb-[72rem] flex-row items-center justify-between">
					...
					<Link to="/" className="duration-200 text-[19rem] font-semibold leading-[140%]">
						Логотип
					</Link>
				</div>
