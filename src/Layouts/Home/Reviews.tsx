	...
	},
	{
		...
		text: (
			<div>
				...
				<span className="text-[#3563E9]">Власовой Ольге Николаевне</span>. Отличный специалист 😊"
			</div>
		),
	},
	{
...
function unflatData(data: ReviewsType[], count: number) {
	const result = [];
	for (let s = 0, e = count; s < data.length; s += count, e += count) result.push(data.slice(s, e));
	return result;
}
...
export default function Reviews() {
	...
	return (
		<div className="relative px-[16rem] md:px-[185rem] flex flex-col mb-[70rem] md:mb-[150rem]">
			...
			<div className="md:hidden snap-mandatory snap-x flex overflow-x-scroll flex-col flex-wrap h-[642rem] gap-x-[16rem]">
				{mobileData.map((element, index) => {
					return (
						<div key={index}>
							<MobileReviews element={element} number={index} setActiveCircle={setActiveCircle} />
						</div>
					);
				})}
			</div>
			...
			<div className="hidden md:flex flex-col-reverse flex-wrap-reverse h-[622rem] gap-[16rem]">
				{data.map((e) => {
					return (
						<div key={e.name}>
							<ReviewCard name={e.name} date={e.date} image={e.image} text={e.text} />
						</div>
					);
				})}
			</div>
